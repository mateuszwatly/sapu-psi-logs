...
\begin{column}{0.46\textwidth}
  ...
  \begin{block}{Sanity check passed}
    \scriptsize TPSAPU is able to learn on MNIST correctly, despite no augmentation. Most models converged around epoch 13.
  \end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{0.48\textwidth}
  ...
  \begin{alertblock}{What doesn't help}
    \begin{itemize}\footnotesize
      \item Spike-only decoders — too noisy for classification, but shows that we can train neurons to spike no matter the tau.
...
\begin{column}{0.48\textwidth}
  \begin{alertblock}{Missing experiments}
    \begin{itemize}\footnotesize
      \item Transformer decoders on MNIST — experimented with one encoder and converged to \>99.5\% almost instantly, but I didn't do a full study.
      \item No pruning applied ($\texttt{prune\_epochs}=0$) — can we achieve \>95\% sparsity on MNIST with every combination?
      \item No documented tau ablation — how much do you lose with $N_\tau=1$?
      \item Backbone baseline not documented, but most smaller pairs stayed at \~15\%, cnn based converged to around 95\%.
    \end{itemize}
  \end{alertblock}
  \vspace{0.3em}
  \begin{tcolorbox}[mybox]
    \scriptsize
    \textbf{Verdict:} TPSAPU achieves MNIST accuracy ($99.4\%$) at $\sim200$k params with sparse binary processing.
    The core architecture works.
    Scaling behaviour on harder tasks is the next question.
  \end{tcolorbox}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[t]{CNN3-702k: Where the Transformer Looks (Aggregate, 500 Samples)}
\vspace{-0.7em}

\begin{columns}[T,totalwidth=\textwidth]

% ── Column 1: main aggregate attention image ─────────────────────────────
\begin{column}{0.47\textwidth}
  ...
  \includegraphics[
    ...
    keepaspectratio
  ]{../visualizations/attention_analysis/cnn3_702k/mean_cls_attention.png}
\end{column}

\hspace{-0.7em}

% ── Column 2: layer comparison image ─────────────────────────────────────
\begin{column}{0.36\textwidth}
  ...
  \includegraphics[
    ...
    height=0.81\textheight,
    keepaspectratio
  ]{../visualizations/attention_analysis/cnn3_702k/layer_comparison.png}
\end{column}

\hspace{0.25em}

% ── Column 3: small interpretation text ──────────────────────────────────
\begin{column}{0.17\textwidth}
  ...
  \begin{tcolorbox}[mybox]
    ...
    \vspace{0.35em}
    CLS attention is broad and close to uniform.

    \vspace{0.45em}
    No single spatial patch dominates the decoder decision.

    \vspace{0.45em}
    Some heads show weak preference for \hl{corners and borders}.

    \vspace{0.45em}
    Layer 2 mildly re-weights attention compared with Layer 1.

    \vspace{0.45em}
    So the transformer mostly aggregates the full TPSAPU reservoir context, rather than selecting one patch.
  \end{tcolorbox}
\end{column}
...
Left: mean CLS attention, std, and per-head attention.
Middle: Layer 1, Layer 2, and L2$-$L1 difference.
}

\end{frame}

% ── SLIDE 13: Per-Image Attention Overlay ────────────────────────────────────
\begin{frame}[shrink=7]{CNN3-702k: Attention Profile Over Token Order}
\begin{columns}[T,onlytextwidth]
...
\begin{column}{0.66\textwidth}
  ...
  \includegraphics[
    ...
    keepaspectratio
  ]{../visualizations/attention_analysis/cnn3_702k/positional_attention_profile.png}

  \vspace{0.1em}
  {\tiny\color{subtletext}
    CNN3 produces a $16\times16$ grid, so token positions $1,\dots,256$
    correspond to spatial CNN patches in fixed scan order.
  }
\end{column}
...
\begin{column}{0.31\textwidth}
  \begin{tcolorbox}[mybox]
    \scriptsize
    \textbf{Continuation of the aggregate view}

    \vspace{0.35em}
    The previous slide showed \emph{where} attention lands on the 2D grid.
    This plot shows the same attention as a 1D token-position profile.

    \vspace{0.55em}
    \textbf{Layer 1:}
    flatter curve, meaning attention is spread broadly across many tokens.

    \vspace{0.45em}
    \textbf{Layer 2:}
    sharper peaks, meaning the second transformer layer becomes more selective.

    \vspace{0.45em}
    The correct-only profile is very similar to the all-sample profile, so this
    is a stable behavior rather than an outlier effect.

    \vspace{0.55em}
    \textbf{Takeaway:}
    CNN3-702k uses broad context, but Layer 2 slightly amplifies selected token
    regions instead of focusing on one dominant patch.
  \end{tcolorbox}
\end{column}
...
\begin{frame}[t,shrink=8]{Training Dynamics: All Three Models}
\vspace{-0.6em}
...
\begin{column}{0.32\textwidth}
  \begin{tcolorbox}[mybox,colframe=accent0!60]
    \tiny
    \orange{ResNet-18}: fast convergence, strong overfit
    {\scriptsize train 99\% vs val 57\%}
  \end{tcolorbox}
\end{column}
...
\begin{column}{0.32\textwidth}
  \begin{tcolorbox}[mybox,colframe=accent1!60]
    \tiny
    \blue{CNN3-702k}: l2-prune dip then recovery;
    less overfit
  \end{tcolorbox}
\end{column}
...
\begin{column}{0.32\textwidth}
  \begin{tcolorbox}[mybox,colframe=accent2!60]
    \tiny
    \green{ResCNN-2048}: 63/85 epochs;
    small batch slows convergence
  \end{tcolorbox}
\end{column}
\end{columns}
...
\includegraphics[
  ...
  keepaspectratio
]{../visualizations/tpsapu_effective_rank/activation_effective_rank.png}
...
\includegraphics[
  ...
  keepaspectratio
]{../visualizations/tpsapu_effective_rank/comparison_with_resnet.png}
...
\begin{column}{0.46\textwidth}
  ...
  \begin{exampleblock}{Why multi-timescale matters especially here}
    ...
    $\tau{=}128$ accumulates motion context over many frames.
